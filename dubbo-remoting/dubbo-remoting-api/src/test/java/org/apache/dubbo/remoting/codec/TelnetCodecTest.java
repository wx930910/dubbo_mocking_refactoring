...
public class TelnetCodecTest {
	...
	protected AbstractMockChannel getServerSideChannel(URL url) {
		url = url
				.addParameter(AbstractMockChannel.LOCAL_ADDRESS,
						url.getAddress())
				.addParameter(AbstractMockChannel.REMOTE_ADDRESS,
						"127.0.0.1:12345");
	...
	protected void testDecode_assertEquals(byte[] request, Object ret)
			throws IOException {
	...
	protected void testDecode_assertEquals(byte[] request, Object ret,
		...
		// init channel
		Channel channel = isServerside ? getServerSideChannel(url)
				: getCliendSideChannel(url);
		// init request string
	...
	protected void testEecode_assertEquals(Object request, byte[] ret,
			boolean isServerside) throws IOException {
		// init channel
		Channel channel = isServerside ? getServerSideChannel(url)
				: getCliendSideChannel(url);
	...
	protected void testDecode_assertEquals(Object request, Object ret)
			throws IOException {
		testDecode_assertEquals(request, ret, null);
	}

	private void testDecode_assertEquals(Object request, Object ret,
			Object channelReceive) throws IOException {
		testDecode_assertEquals(null, request, ret, channelReceive);
	}

	private void testDecode_assertEquals(AbstractMockChannel channel,
			Object request, Object expectret, Object channelReceive)
			throws IOException {
		// init channel
		if (channel == null) {
	...
	private void testDecode_PersonWithEnterByte(byte[] enterbytes,
			boolean isNeedmore) throws IOException {
		// init channel
	...
	private void testDecode_WithExitByte(byte[] exitbytes,
			boolean isChannelClose) throws IOException {
		// init channel
		Channel channel = getServerSideChannel(url);
	...
	public void testDecode_BlankMessage() throws IOException {
		testDecode_assertEquals(new byte[] {},
	...
	public void testDecode_Person_ObjectOnly() throws IOException {
		testDecode_assertEquals(new Person(),
				Codec2.DecodeResult.NEED_MORE_INPUT);
	}
	...
	public void testDecode_WithExitByte() throws IOException {
		...
		exitbytes.put(new byte[] { -1, -12, -1, -3, 6 },
				true); /* Linux Ctrl+C */
		exitbytes.put(new byte[] { 1, -1, -12, -1, -3, 6 }, false); // must
																	// equal the
																	// bytes
		exitbytes.put(new byte[] { -1, -19, -1, -3, 6 },
				true); /* Linux Pause */
	...
	public void testDecode_Backspace() throws IOException {
		...
		// recognition). Ignoring this problem, the backspace key is only
		// meaningfully input in a real telnet program.
		testDecode_assertEquals(new byte[] { 'a', 'x', -1, 'x', '\b' },
				Codec2.DecodeResult.NEED_MORE_INPUT,
				new String(new byte[] { 32, 32, 8, 8 }));
	}
	...
	public void testDecode_Backspace_WithError() throws IOException {
		Assertions.assertThrows(IOException.class, () -> {
			url = url.addParameter(AbstractMockChannel.ERROR_WHEN_SEND,
	...
	public void testDecode_History_UP() throws IOException {
		...
		testDecode_assertEquals(channel, UP,
				Codec2.DecodeResult.NEED_MORE_INPUT, null);
		...
		testDecode_assertEquals(channel, UP,
				Codec2.DecodeResult.NEED_MORE_INPUT, expected1);
	}
