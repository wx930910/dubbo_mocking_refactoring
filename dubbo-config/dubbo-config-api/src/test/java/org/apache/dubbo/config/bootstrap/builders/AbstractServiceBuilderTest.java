...
class AbstractServiceBuilderTest {
	...
	void document() {
		...
		Assertions.assertEquals("http://dubbo.apache.org",
				builder.build().getDocument());
	}
	...
	void protocolIds() {
		...
		Assertions.assertEquals("protocolIds",
				builder.build().getProtocolIds());
	}
	...
	void serialization() {
		...
		Assertions.assertEquals("serialization",
				builder.build().getSerialization());
	}
	...
	void build() {
		...
		ServiceBuilder builder = new ServiceBuilder();
		builder.version("version").group("group").deprecated(true).delay(1000)
				.export(false).weight(1).document("document").dynamic(true)
				.token("token").accesslog("accesslog").addProtocol(protocol)
				.protocolIds("protocolIds").tag("tag").executes(100)
				.register(false).warmup(200).serialization("serialization")
				.id("id").prefix("prefix");
	...
	private static class ServiceBuilder extends
			AbstractServiceBuilder<AbstractServiceConfig, ServiceBuilder> {

		public AbstractServiceConfig build() {
			AbstractServiceConfig parameterConfig = new MockServiceConfig().instance;
	...
	private static class MockServiceConfig {
		public AbstractServiceConfig instance = Mockito
				.mock(AbstractServiceConfig.class, Mockito.CALLS_REAL_METHODS);
	}
