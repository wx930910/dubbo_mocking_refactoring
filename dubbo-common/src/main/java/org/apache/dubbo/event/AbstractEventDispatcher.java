...
public abstract class AbstractEventDispatcher implements EventDispatcher {
	 ...
	 *             <code>executor</code> is <code>null</code>
	 */
	...
	protected Stream<EventListener> sortedListeners(
			Predicate<Map.Entry<Class<? extends Event>, List<EventListener>>> predicate) {
		return listenersCache.entrySet().stream().filter(predicate).map(Map.Entry::getValue).flatMap(Collection::stream)
				.sorted();
	}
	...
	/**
	 * Default, load the instances of {@link EventListener event listeners} by
	 * {@link ServiceLoader}
	 * <p>
