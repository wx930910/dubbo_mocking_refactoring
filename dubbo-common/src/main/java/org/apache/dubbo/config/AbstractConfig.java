...
public abstract class AbstractConfig implements Serializable {
	...
	private static boolean isParametersGetter(Method method) {
		String name = method.getName();
		return ("getParameters".equals(name) && Modifier.isPublic(method.getModifiers()) && method.getParameterTypes().length == 0
				&& method.getReturnType() == Map.class);
	}

	private static boolean isParametersSetter(Method method) {
		return ("setParameters".equals(method.getName()) && Modifier.isPublic(method.getModifiers()) && method.getParameterCount() == 1
				&& Map.class == method.getParameterTypes()[0] && method.getReturnType() == void.class);
	}
	...
	private static Map<String, String> convert(Map<String, String> parameters, String prefix) {
		...
		for (Map.Entry<String, String> entry : parameters.entrySet()) {
			...
			// For compatibility, key like "registry-type" will has a duplicate
			// key "registry.type"
			if (key.contains("-")) {
	...
	protected void appendAnnotation(Class<?> annotationClass, Object annotation) {
		...
		for (Method method : methods) {
			if (method.getDeclaringClass() != Object.class && method.getReturnType() != void.class && method.getParameterTypes().length == 0
	...
	/**
	 * Should be called after Config was fully initialized. // FIXME: this
	 * method should be completely replaced by appendParameters
	 *
	 * @return
		...
		for (Method method : methods) {
			try {
				...
				if (MethodUtils.isMetaMethod(method)) {
					...
					// treat url and configuration differently, the value should
					// always present in configuration though it may not need to
					// present in url.
					...
					/**
					 * Attributes annotated as deprecated should not override
			...
			for (Method method : methods) {
				if (MethodUtils.isSetter(method)) {
					try {
						...
						logger.info("Failed to override the property " + method.getName() + " in " + this.getClass().getSimpleName()
	 ...
	 * <p>
	 * Current method will invoked by Spring or Java EE container automatically,
	 * or should be triggered manually.
	 *
